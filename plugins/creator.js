let handler = async(m, { conn }) => {
  ...
  const sentMsg = await conn.sendContactArray(m.chat, [
    [`${owner[0]}`, `${await conn.getName(owner[0]+'@s.whatsapp.net')}`, `👑 Developer Bot `, `🚫 Don't call me 🥺`, `sabooygy@gmail.com`, `🇮🇩 Indonesia`, `🚀 https://chat.whatsapp.com/BXRH5ApRnbaHm6ULbPuG4d`, `punya pawang banh 🗿`],
    [`${conn.user.jid.split('@')[0]}`, `${await conn.getName(conn.user.jid)}`, `🔥 Bot WhatsApp 🐣`, `📵 Don't spam/call me 😢`, `Nothing`, `🇮🇩 Indonesia`, `🚀 https://chat.whatsapp.com/BXRH5ApRnbaHm6ULbPuG4d`, `🤖 Hanya bot biasa yang kadang suka eror ☺`]
  ], fkonn) 
